#!/bin/bash
#SBATCH --job-name=260224_38b_LoRA_sft_crit_mantis_only_en
...
export MAX_PROMPT_LOGS=20  # Log first 20 training prompts (default: 10)
...
GLOBAL_BATCH_SIZE=${GLOBAL_BATCH_SIZE:-128}  # Target global batch size (8 node × 8 GPUs × 1 per-device × 2 grad_acc = 128)
...
# Output directories
EXP_NAME=260224_38b_LoRA_sft_crit_mantis_only_en
...
srun --ntasks-per-node=1 \
  ...
  bash -c '
    ...
    torchrun \
      ...
      --model_name_or_path "/fsx/VLM/internvl3_5/code/InternVL-debug/internvl_chat_gpt_oss/work_dirs/checkpoint-7000-merged-gold-weight" \
      --conv_style "'"$CONV_STYLE"'" \
      --use_fast_tokenizer False \
      --output_dir "'"$OUTPUT_DIR"'" \
      --meta_path "./shell/data/crit_mantis.json" \
