#!/bin/bash
#SBATCH --job-name=260210_38b_LoRA_sft_stage2_only_full
...
GLOBAL_BATCH_SIZE=${GLOBAL_BATCH_SIZE:-512}  # Target global batch size (8 node × 8 GPUs × 2 per-device × 4 grad_acc = 512)
...
EXP_NAME=260210_38b_LoRA_sft_stage2_only_full
...
srun --ntasks-per-node=1 \
  ...
  bash -c '
    ...
    torchrun \
      ...
      --model_name_or_path "/fsx/VLM/internvl3_5/code/InternVL-debug/internvl_chat_gpt_oss/work_dirs/checkpoint-26000-merged-gemini" \
      --conv_style "'"$CONV_STYLE"'" \
      --use_fast_tokenizer False \
      --output_dir "'"$OUTPUT_DIR"'" \
      --meta_path "./shell/data/crit_bi_mantis.json" \
